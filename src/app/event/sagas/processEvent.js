...
function* processOccurrences(eventCount, eventId, defaultStartTime, eventType) {
  try {
    const userToken = localStorage.token;
    const occurrences = createOccurrences(eventCount, eventId, defaultStartTime, eventType);

    const res = yield axios.post(`occurrences.json?auth=${userToken}`, occurrences);
